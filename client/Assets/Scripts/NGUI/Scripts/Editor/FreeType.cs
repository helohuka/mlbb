...
static public class FreeType
{
			...
			if (!mFound)
			{
				...
				else
				{
					...
					if (File.Exists(path))
					{
						...
						{
							ClientLog.Instance.LogWarning("Unable to copy FreeType.dylib to /usr/local/lib:\n" + ex.Message);
						}
					}
				}
			}
	...
	static public string[] GetFaces (Font font)
	{
		...
		{
			ClientLog.Instance.LogError("Failed to initialize FreeType");
			return null;
		}
		...
		if (File.Exists(fileName))
		{
			...
			{
				ClientLog.Instance.LogError("Unable to use the chosen font (FT_New_Face).");
			}
			else
			{
	...
	static public bool CreateFont (Font ttf, int size, int faceIndex, string characters, out BMFont font, out Texture2D tex)
	{
		...
		if (!File.Exists(fileName))
		{
			ClientLog.Instance.LogError("Unable to use the chosen font.");
		}
		...
		else
		{
			...
			//ClientLog.Instance.Log(ascender + " " + descender + " " + baseline);
