...
public class UILabel : UIWidget
{
	...
	[HideInInspector][SerializeField] protected Font mTrueTypeFont;
	...
	[HideInInspector][SerializeField] protected int mFontSize = 16;
	[HideInInspector][SerializeField] protected FontStyle mFontStyle = FontStyle.Normal;
	[HideInInspector][SerializeField] protected Alignment mAlignment = Alignment.Automatic;
	...
	[HideInInspector][SerializeField] protected NGUIText.SymbolStyle mSymbols = NGUIText.SymbolStyle.Normal;
	[HideInInspector][SerializeField] protected Vector2 mEffectDistance = Vector2.one;
	[HideInInspector][SerializeField] protected Overflow mOverflow = Overflow.ShrinkContent;
	[HideInInspector][SerializeField] protected Material mMaterial;
	...
	[HideInInspector][SerializeField] protected int mSpacingX = 0;
	[HideInInspector][SerializeField] protected int mSpacingY = 0;
    ...
    public string GetInfoAtPosition(Vector3 worldPos) { return GetInfoAtCharacterIndex(GetCharacterIndexAtPosition(worldPos)); }

    /// <summary>
    /// Retrieve the URL directly below the specified relative-to-label position.
    /// </summary>

    public string GetInfoAtPosition(Vector2 localPos) { return GetInfoAtCharacterIndex(GetCharacterIndexAtPosition(localPos)); }

    /// <summary>
    /// Retrieve the URL right under the specified character index.
    /// </summary>

    public string GetInfoAtCharacterIndex(int characterIndex)
    {
        if (characterIndex != -1 && characterIndex <= mText.Length)
        {
            int linkStart = mText.LastIndexOf("[info=", characterIndex);
            ...
            if (linkStart != -1)
            {
                ...
                if (linkEnd != -1)
                {
                //    int closingStatement = mText.IndexOf("[/info]", linkEnd);
                //    if (closingStatement == -1 || closingStatement >= characterIndex)
                        return mText.Substring(linkStart, linkEnd - linkStart);
                }
            }
