...
public class UISprite : UIBasicSprite
{
	...
	public UISpriteData GetAtlasSprite ()
	{
		...
		if (mSprite == null && mAtlas != null)
		{
			...
			if (mSprite == null && mAtlas.spriteList.Count > 0)
			{
				...
				if (mSprite == null)
				{
					ClientLog.Instance.LogError(mAtlas.name + " seems to have a null sprite!");
					return null;
				}
				mSpriteName = mSprite.name;
			}
	...
	protected override void OnUpdate ()
	{
        ...
        if (mark_ != null && mark_.activeSelf)
        {
            mark_.transform.localPosition = position_;
            mark_.transform.localRotation = rotation_;
        }
	}
    ...
    public void MarkOn(MarkAnthor anthor = MarkAnthor.MA_RightTop, float offsetX = 0f, float offsetY = 0f)
    {
        Vector3 position = Vector3.zero;
        if (mark_ == null)
        {
            mark_ = new GameObject("mark");
            mark_.layer = LayerMask.NameToLayer("UI");

            UITexture tex = mark_.GetComponent<UITexture>();
            if (tex == null)
                tex = mark_.AddComponent<UITexture>();

            if (texture_ == null)
            {
                texture_ = Resources.Load<Texture2D>("markSign");
            }

            tex.mainTexture = texture_;
            tex.depth = depth + 3;
            tex.width = texture_.width;
            tex.height = texture_.height;

            mark_.transform.parent = transform;
            mark_.transform.localScale = Vector3.one;
            switch(anthor)
            {
                case MarkAnthor.MA_LeftTop:
                    position = new Vector3(width / -2f + offsetX, height / 2f + offsetY, 0f);
                    break;
                case MarkAnthor.MA_LeftCenter:
                    position = new Vector3(width / -2f + offsetX, offsetY, 0f);
                    break;
                case MarkAnthor.MA_LeftBottom:
                    position = new Vector3(width / -2f + offsetX, height / -2f + offsetY, 0f);
                    break;
                case MarkAnthor.MA_RightTop:
                    position = new Vector3(width / 2f + offsetX, height / 2f + offsetY, 0f);
                    break;
                case MarkAnthor.MA_RightCenter:
                    position = new Vector3(width / 2f + offsetX, offsetY, 0f);
                    break;
                case MarkAnthor.MA_RightBottom:
                    position = new Vector3(width / 2f + offsetX, height / -2f + offsetY, 0f);
                    break;
                case MarkAnthor.MA_Top:
                    position = new Vector3(offsetX, height / 2f + offsetY, 0f);
                    break;
                case MarkAnthor.MA_Center:
                    position = new Vector3(offsetX, height / 2f + offsetY, 0f);
                    break;
                case MarkAnthor.MA_Bottom:
                    position = new Vector3(offsetX, height / 2f + offsetY, 0f);
                    break;
                default:
                    break;
            }
            mark_.transform.localPosition = position;
            position_ = mark_.transform.localPosition;
            rotation_ = mark_.transform.localRotation;
        }
        else
		{
			if(!mark_.gameObject.activeSelf)
            	mark_.SetActive(true);
		}
    }
