...
public class EventDelegate
{
	...
	void Cache ()
	{
		...
		if (mCachedCallback == null || (mCachedCallback.Target as MonoBehaviour) != mTarget || GetMethodName(mCachedCallback) != mMethodName)
		{
			if (mTarget != null && !string.IsNullOrEmpty(mMethodName))
			{
				...
				{
					ClientLog.Instance.LogError("Failed to bind " + type + "." + mMethodName + "\n" +  ex.Message);
					return;
				}

				if (mMethod == null)
				{
					ClientLog.Instance.LogError("Could not find method '" + mMethodName + "' on " + mTarget.GetType(), mTarget);
					return;
				}
				
				if (mMethod.ReturnType != typeof(void))
				{
					ClientLog.Instance.LogError(mTarget.GetType() + "." + mMethodName + " must have a 'void' return type.", mTarget);
					return;
				}
		...
		if (mMethod != null)
		{
			...
			else
			{
				...
				catch (System.ArgumentException ex)
				{
					...
					msg += "\n";
					ClientLog.Instance.LogError(msg);
				}
	...
	static public void Add (List<EventDelegate> list, EventDelegate ev, bool oneShot)
	{
		...
		else ClientLog.Instance.LogWarning("Attempting to add a callback to a list that's null");
	}
