...
public static class Localization
{
	...
	static public bool LoadCSV (TextAsset asset)
	{
		...
		{
			ClientLog.Instance.LogError("Invalid localization CSV file. The first value is expected to be 'KEY', followed by language columns.\n" +
	...
	static void AddCSV (BetterList<string> values)
	{
		...
		{
			ClientLog.Instance.LogError("Unable to add '" + values[0] + "' to the Localization dictionary.\n" + ex.Message);
		}
	}
...
#if UNITY_EDITOR
		ClientLog.Instance.LogWarning("Localization key not found: '" + key + "'");
#endif
		return key;
	}
