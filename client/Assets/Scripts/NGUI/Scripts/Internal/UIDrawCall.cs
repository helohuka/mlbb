...
public class UIDrawCall : MonoBehaviour
{
	...
	void CreateMaterial ()
	{
		...
		else ClientLog.Instance.LogError(shaderName + " shader doesn't have a clipped shader version for " + mClipCount + " clip regions");
	}
	...
	void UpdateMaterials ()
	{
		...
		{
#if UNITY_EDITOR
			ClientLog.Instance.LogError("Hmm... This point got hit!");
#endif
	...
	public void UpdateGeometry ()
	{
		...
		if (count > 0 && (count == uvs.size && count == cols.size) && (count % 4) == 0)
		{
			...
			else
			{
				...
				ClientLog.Instance.LogError("Too many vertices on one panel: " + verts.size);
			}
		...
		{
			if (mFilter.mesh != null) mFilter.mesh.Clear();
			ClientLog.Instance.LogError("UIWidgets must fill the buffer with 4 vertices per quad. Found " + count);
		}
				...
				if (currentPanel.hasClipping)
				{
					...
					if (currentPanel != panel)
					{
						...
						if (Mathf.Abs(diff.x) > 0.001f || Mathf.Abs(diff.y) > 0.001f)
							ClientLog.Instance.LogWarning("Panel can only be clipped properly if X and Y rotation is left at 0", panel);
