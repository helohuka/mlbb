...
namespace UniLua
{
	...
	public class LuaTable {
		...
		public void SetInt(int key, ref TValue val)
		{
			...
			cell.V.SetObj(ref val);
			// ClientLog.Instance.Log(string.Format("---------------- SetInt {0} -> {1}", key, val));
			// DumpParts();
		}
		...
		*/
		...
		{
			ClientLog.Instance.Log("------------------ [DumpParts] enter -----------------------");
			ClientLog.Instance.Log("<< Array Part >>");
			for(var i=0; i<ArrayPart.Length; ++i) {
				var n = ArrayPart[i];
				ClientLog.Instance.Log(string.Format("i:{0} val:{1}", i, n.V));
			}
			ClientLog.Instance.Log("<< Hash Part >>");
			for(var i=0; i<HashPart.Length; ++i) {
				var n = HashPart[i];
				var next = (n.Next == null) ? -1 : n.Next.Index;
				ClientLog.Instance.Log(string.Format("i:{0} index:{1} key:{2} val:{3} next:{4}", i, n.Index, n.Key.V, n.Val.V, next));
			}
			ClientLog.Instance.Log("++++++++++++++++++ [DumpParts] leave +++++++++++++++++++++++");
		}
