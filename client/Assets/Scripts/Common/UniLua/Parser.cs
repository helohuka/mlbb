...
namespace UniLua
{
	...
	public class Parser
	{
		...
		{
			// ClientLog.Instance.Log("Statement ::" + Lexer.Token);
			int line = Lexer.LineNumber;
			EnterLevel();
			switch( Lexer.Token.TokenType )
			{
			...
			// ClientLog.Instance.Log( "MaxStackSize: " + CurFunc.Proto.MaxStackSize );
			// ClientLog.Instance.Log( "FreeReg: " + CurFunc.FreeReg );
			// ClientLog.Instance.Log( "NumActVar: " + CurFunc.NumActVar );
		...
		{
			// ClientLog.Instance.Log( Lexer.Token );
			var t = Lexer.Token as NameToken;

			// TEST CODE
			if( t == null )
			{
				ClientLog.Instance.LogError( Lexer.LineNumber + ":" + Lexer.Token );
			}
		...
		{
			// ClientLog.Instance.Log("SubExpr limit:" + limit);
			EnterLevel();
