...
public class moreActivityOneUI : MonoBehaviour
{
	...
	void Start ()
	{
		MoreActivityData.instance.SysChargeEveryEvent += new RequestEventHandler<COM_ADChargeEvery> (OnUpdataEvent);
		UpdateInfo ();
	}
	...
	void UpdateInfo()
	{
		COM_ADChargeEvery data = MoreActivityData.instance.GetSysChargeEvery ();
		if (data == null)
			return;
	...
	{
		NetConnection.Instance.requestChargeEveryReward((uint)param1);
	}


	void OnDestroy()
	{
		MoreActivityData.instance.SysChargeEveryEvent -= OnUpdataEvent;
	}
}
