...
public class MoreSelfDiscountStoreUI : MonoBehaviour
{
	...
	void Start ()
	{
		MoreActivityData.instance.SelfDiscountStoreEvent += new RequestEventHandler<COM_ADDiscountStore> (OnUpdataEvent);
	...
	void UpdateInfo ()
	{
		COM_ADDiscountStore data = MoreActivityData.instance.GetSelfDiscountStore ();
		...
		for(int i =0;i<data.contents_.Length;i++)
		{
			...
			cellUI.needMoney.text = ((int)((float)data.contents_[i].price_*data.contents_[i].discount_)).ToString();
	...
	private void OnBuyBtn(ButtonScript obj, object args, int param1, int param2)
	{
		...
		MessageBoxUI.ShowMe (LanguageManager.instance.GetValue ("shopbuyitem").Replace ("{n}", count .ToString ()).Replace ("{n1}", stips.nameLabel.text).Replace ("{n2}", (count * int.Parse(stips.jiageLabel.text)).ToString ()+LanguageManager.instance.GetValue ("shuijing")), () => {
			
			NetConnection.Instance.buyDiscountStoreSingle (int.Parse (Tips.name), count);});
	}

	void OnDestroy()
	{
		MoreActivityData.instance.SelfDiscountStoreEvent -= OnUpdataEvent;
	}
}
