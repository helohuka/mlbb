...
public class moreActivitySelfTolalUI : MonoBehaviour
{
	...
	void Start ()
	{
		MoreActivityData.instance.SelfRechargeEvent += new RequestEventHandler<COM_ADChargeTotal> (OnUpdataEvent);
		UpdateInfo ();
	}
	...
	{
		NetConnection.Instance.requestChargeTotalSingleReward ((uint)param1);
	}

	void OnDestroy()
	{
		MoreActivityData.instance.SelfRechargeEvent -= OnUpdataEvent;
	}
}
