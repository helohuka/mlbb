...
struct CrowdAgent
{
	...
	CrowdNeighbour neis[DT_CROWDAGENT_MAX_NEIGHBOURS];
...
class Crowd
{
	int maxAgents_;
	CrowdAgent* agents_;
	CrowdAgent** activeAgents_;
	CrowdAgentAnimation* agentAnims_;
	
	dtPathQueue pathq_;

	dtObstacleAvoidanceParams obstacleQueryParams_[DT_CROWD_MAX_OBSTAVOIDANCE_PARAMS];
	dtObstacleAvoidanceQuery* obstacleQuery_;
	
	dtProximityGrid* grid_;
	
	dtPolyRef* pathResult_;
	int maxPathResult_;
	
	float ext_[3];

	dtQueryFilter filters_[DT_CROWD_MAX_QUERY_FILTER_TYPE];

	float maxAgentRadius_;

	int velocitySampleCount_;

	NavMeshQuery* navquery_;

	void updateTopologyOptimization(CrowdAgent** agents, const int nagents, const float dt);
	void updateMoveRequest(const float dt);
	void checkPathValidity(CrowdAgent** agents, const int nagents, const float dt);

	inline int getAgentIndex(const CrowdAgent* agent) const  { return (int)(agent - agents_); }
...
public:
	...
	const CrowdAgent* getAgent(const int idx);

	/// Gets the specified agent from the pool.
	///	 @param[in]		idx		The agent index. [Limits: 0 <= value < #getAgentCount()]
	/// @return The requested agent.
	CrowdAgent* getEditableAgent(const int idx);
	...
	int addAgent(const float* pos, const CrowdAgentParams* params);

	/// Updates the specified agent's configuration.
	///  @param[in]		idx		The agent index. [Limits: 0 <= value < #getAgentCount()]
	///  @param[in]		params	The new agent configuration.
	void updateAgentParameters(const int idx, const CrowdAgentParams* params);
	...
	int getActiveAgents(CrowdAgent** agents, const int maxAgents);

	/// Updates the steering and positions of all agents.
	///  @param[in]		dt		The time, in seconds, to update the simulation. [Limit: > 0]
	///  @param[out]	debug	A debug object to load with debug information. [Opt]
	void update(const float dt, CrowdAgentDebugInfo* debug);
	
	/// Gets the filter used by the crowd.
	/// @return The filter used by the crowd.
	inline const dtQueryFilter* getFilter(const int i) const { return (i >= 0 && i < DT_CROWD_MAX_QUERY_FILTER_TYPE) ? &filters_[i] : 0; }
	
	/// Gets the filter used by the crowd.
	/// @return The filter used by the crowd.
	inline dtQueryFilter* getEditableFilter(const int i) { return (i >= 0 && i < DT_CROWD_MAX_QUERY_FILTER_TYPE) ? &filters_[i] : 0; }

	/// Gets the search extents [(x, y, z)] used by the crowd for query operations. 
	/// @return The search extents used by the crowd. [(x, y, z)]
	const float* getQueryExtents() const { return ext_; }
	
	/// Gets the velocity sample count.
	/// @return The velocity sample count.
	inline int getVelocitySampleCount() const { return velocitySampleCount_; }
	
	/// Gets the crowd's proximity grid.
	/// @return The crowd's proximity grid.
	const dtProximityGrid* getGrid() const { return grid_; }

	/// Gets the crowd's path request queue.
	/// @return The crowd's path request queue.
	const dtPathQueue* getPathQueue() const { return &pathq_; }

	/// Gets the query object used by the crowd.
	const NavMeshQuery* getNavMeshQuery() const { return navquery_; }
};
