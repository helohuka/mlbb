...
static void mergeRegionHoles(rcContourRegion& region)
{
	...
	if (!diags)
	{
		ACE_DEBUG((LM_WARNING, "mergeRegionHoles: Failed to allocated diags %d.\n", maxVerts));
		return;
	}
	...
	for (int i = 0; i < region.nholes; i++)
	{
		...
		if (index == -1)
		{
			ACE_DEBUG((LM_WARNING, "mergeHoles: Failed to find merge points for %p and %p.\n", region.outline, hole));
			continue;
		}
		if (!mergeContours(*region.outline, *hole, index, bestVertex))
		{
			ACE_DEBUG((LM_WARNING, "mergeHoles: Failed to merge contours %p and %p.\n", region.outline, hole));
			continue;
		}
	}
}
					 ...
					 rcContourSet& cset, const int buildFlags)
{
	...
	if (!flags)
	{
		ACE_DEBUG((LM_ERROR, "rcBuildContours: Out of memory 'flags' (%d).\n", chf.spanCount));
		return false;
	}
			...
			for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; ++i)
			{
				...
				if (simplified.size()/4 >= 3)
				{
					if (cset.nconts >= maxContours)
					{
						...
						ACE_DEBUG((LM_WARNING,  "rcBuildContours: Expanding max contours from %d to %d.\n", oldMax, maxContours));
					}
					...
					if (!cont->verts)
					{
						ACE_DEBUG((LM_ERROR, "rcBuildContours: Out of memory 'verts' (%d).\n", cont->nverts));
						return false;
					}
					...
					if (!cont->rverts)
					{
						ACE_DEBUG((LM_ERROR, "rcBuildContours: Out of memory 'rverts' (%d).\n", cont->nrverts));
						return false;
					}
	...
	if (cset.nconts > 0)
	{
		...
		if (!winding)
		{
			ACE_DEBUG((LM_ERROR, "rcBuildContours: Out of memory 'hole' (%d).\n", cset.nconts));
			return false;
		}
		int nholes = 0;
		...
		if (nholes > 0)
		{
			...
			if (!regions)
			{
				ACE_DEBUG((LM_ERROR, "rcBuildContours: Out of memory 'regions' (%d).\n", nregions));
				return false;
			}
			...
			if (!holes)
			{
				ACE_DEBUG((LM_ERROR, "rcBuildContours: Out of memory 'holes' (%d).\n", cset.nconts));
				return false;
			}
			...
			for (int i = 0; i < cset.nconts; ++i)
			{
				...
				if (winding[i] > 0)
				{
					if (regions[cont.reg].outline)
						ACE_DEBUG((LM_ERROR, "rcBuildContours: Multiple outlines for region %d.\n", cont.reg));
			...
			for (int i = 0; i < nregions; i++)
			{
				...
				else
				{
					...
					ACE_DEBUG((LM_ERROR, "rcBuildContours: Bad outline for region %d, contour simplification is likely too aggressive.\n", i));
				}
			}
		}
