...
bool duDumpPolyMeshToObj(rcPolyMesh& pmesh, duFileIO* io)
{
	if (!io)
	{
		ACE_DEBUG((LM_ERROR,"duDumpPolyMeshToObj: input IO is null.\n")); 
		return false;
	}
	if (!io->isWriting())
	{
		ACE_DEBUG((LM_ERROR,"duDumpPolyMeshToObj: input IO not writing.\n")); 
		return false;
	}
...
{
	if (!fp_ || mode_ != 2) return false;
	size_t readLen = fread(ptr, size, 1, fp_);
	return readLen == 1;
}
