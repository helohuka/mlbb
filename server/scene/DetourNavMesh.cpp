	...
	tiles_(0)
{
	...
	polyBits_ = 0;
#endif
	memset(&params_, 0, sizeof(dtNavMeshParams));
	orig_[0] = 0;
	orig_[1] = 0;
	orig_[2] = 0;
}
...
dtNavMesh::~dtNavMesh()
{
	...
	{
		if (tiles_[i].flags & DT_TILE_FREE_DATA)
		{
...
{
	memcpy(&params_, params, sizeof(dtNavMeshParams));
	dtVcopy(orig_, params->orig);
	tileWidth_ = params->tileWidth;
	tileHeight_ = params->tileHeight;
	
	// Init tiles
	maxTiles_ = params->maxTiles;
	tileLutSize_ = dtNextPow2(params->maxTiles/4);
	if (!tileLutSize_) tileLutSize_ = 1;
	tileLutMask_ = tileLutSize_-1;
	
	tiles_ = (dtMeshTile*)dtAlloc(sizeof(dtMeshTile)*maxTiles_, DT_ALLOC_PERM);
	if (!tiles_)
		return DT_FAILURE | DT_OUT_OF_MEMORY;
	posLookup_ = (dtMeshTile**)dtAlloc(sizeof(dtMeshTile*)*tileLutSize_, DT_ALLOC_PERM);
	if (!posLookup_)
		return DT_FAILURE | DT_OUT_OF_MEMORY;
	memset(tiles_, 0, sizeof(dtMeshTile)*maxTiles_);
	memset(posLookup_, 0, sizeof(dtMeshTile*)*tileLutSize_);
	nextFree_ = 0;
	for (int i = maxTiles_-1; i >= 0; --i)
	{
		tiles_[i].salt = 1;
		tiles_[i].next = nextFree_;
		nextFree_ = &tiles_[i];
	}
	
	// Init ID generator values.
#ifndef DT_POLYREF64
	tileBits_ = dtIlog2(dtNextPow2((unsigned int)params->maxTiles));
	polyBits_ = dtIlog2(dtNextPow2((unsigned int)params->maxPolys));
	// Only allow 31 salt bits, since the salt mask is calculated using 32bit uint and it will overflow.
	saltBits_ = dtMin((unsigned int)31, 32 - tileBits_ - polyBits_);
							...
							dtTileRef lastRef, dtTileRef* result)
{
	...
	else
	{
		...
		if (tileIndex >= maxTiles_)
			return DT_FAILURE | DT_OUT_OF_MEMORY;
		// Try to find the specific tile id from the free list.
		dtMeshTile* target = &tiles_[tileIndex];
		dtMeshTile* prev = 0;
		tile = nextFree_;
	...
	int h = computeTileHash(header->x, header->y, tileLutMask_);
...
const dtMeshTile* dtNavMesh::getTileAt(const int x, const int y, const int layer) const
{
	...
	int h = computeTileHash(x,y,tileLutMask_);
	dtMeshTile* tile = posLookup_[h];
	while (tile)
	{
		if (tile->header &&
			tile->header->x == x &&
...
const dtMeshTile* dtNavMesh::getTileByRef(dtTileRef ref) const
{
	...
	if ((int)tileIndex >= maxTiles_)
		return 0;
	const dtMeshTile* tile = &tiles_[tileIndex];
...
{
	*tx = (int)floorf((pos[0]-orig_[0]) / tileWidth_);
	*ty = (int)floorf((pos[2]-orig_[2]) / tileHeight_);
}
...
dtStatus dtNavMesh::getTileAndPolyByRef(const dtPolyRef ref, const dtMeshTile** tile, const dtPoly** poly) const
{
	...
	if (it >= (unsigned int)maxTiles_) return DT_FAILURE | DT_INVALID_PARAM;
	if (tiles_[it].salt != salt || tiles_[it].header == 0) return DT_FAILURE | DT_INVALID_PARAM;
	if (ip >= (unsigned int)tiles_[it].header->polyCount) return DT_FAILURE | DT_INVALID_PARAM;
	*tile = &tiles_[it];
	*poly = &tiles_[it].polys[ip];
	return DT_SUCCESS;
}
...
bool dtNavMesh::isValidPolyRef(dtPolyRef ref) const
{
	...
	unsigned int salt, it, ip;
	decodePolyId(ref, salt, it, ip);
	if (it >= (unsigned int)maxTiles_) return false;
	if (tiles_[it].salt != salt || tiles_[it].header == 0) return false;
	if (ip >= (unsigned int)tiles_[it].header->polyCount) return false;
	return true;
}
...
dtStatus dtNavMesh::removeTile(dtTileRef ref, unsigned char** data, int* dataSize)
{
	...
	if ((int)tileIndex >= maxTiles_)
		return DT_FAILURE | DT_INVALID_PARAM;
	dtMeshTile* tile = &tiles_[tileIndex];
	if (tile->salt != tileSalt)
		return DT_FAILURE | DT_INVALID_PARAM;
	
	// Remove tile from hash lookup.
	int h = computeTileHash(tile->header->x,tile->header->y,tileLutMask_);
	dtMeshTile* prev = 0;
	dtMeshTile* cur = posLookup_[h];
	while (cur)
	{
		if (cur == tile)
		{
...
dtStatus dtNavMesh::getOffMeshConnectionPolyEndPoints(dtPolyRef prevRef, dtPolyRef polyRef, float* startPos, float* endPos) const
{
	...
	if (it >= (unsigned int)maxTiles_) return DT_FAILURE | DT_INVALID_PARAM;
	if (tiles_[it].salt != salt || tiles_[it].header == 0) return DT_FAILURE | DT_INVALID_PARAM;
	const dtMeshTile* tile = &tiles_[it];
...
const dtOffMeshConnection* dtNavMesh::getOffMeshConnectionByRef(dtPolyRef ref) const
{
	...
	// Get current polygon
	decodePolyId(ref, salt, it, ip);
	if (it >= (unsigned int)maxTiles_) return 0;
	if (tiles_[it].salt != salt || tiles_[it].header == 0) return 0;
	const dtMeshTile* tile = &tiles_[it];
	...
	SRV_ASSERT(idx < (unsigned int)tile->header->offMeshConCount);
