...
static int addEdge(int* edges, int& nedges, const int maxEdges, int s, int t, int l, int r)
{
	if (nedges >= maxEdges)
	{
		ACE_DEBUG((LM_ERROR,"addEdge: Too many edges (%d/%d).\n", nedges, maxEdges));
		return EV_UNDEF;
	}
						 ...
						 rcIntArray& tris, rcIntArray& edges)
{
	...
	for (int i = 0, j = nhull-1; i < nhull; j=i++)
		addEdge( &edges[0], nedges, maxEdges, hull[j],hull[i], EV_HULL, EV_UNDEF);
	...
	for (int i = 0; i < tris.size()/4; ++i)
	{
		...
		{
			ACE_DEBUG((LM_WARNING,"delaunayHull: Removing dangling face %d [%d,%d,%d].\n", i, t[0],t[1],t[2]));
							...
							rcIntArray& edges, rcIntArray& samples)
{
	...
	if (tris.size() == 0)
	{
		...
		ACE_DEBUG((LM_WARNING, "buildPolyDetail: Could not triangulate polygon (%d verts).\n", nverts));
		return true;
	}
	...
	if (ntris > MAX_TRIS)
	{
		...
		ACE_DEBUG((LM_ERROR, "rcBuildPolyMeshDetail: Shrinking triangle count from %d to max %d.\n", ntris, MAX_TRIS));
	}
						   ...
						   rcPolyMeshDetail& dmesh)
{
	...
	if (!bounds)
	{
		ACE_DEBUG((LM_ERROR,"rcBuildPolyMeshDetail: Out of memory 'bounds' (%d).\n", mesh.npolys*4));
		return false;
	}
	rcScopedDelete<float> poly = (float*)rcAlloc(sizeof(float)*nvp*3, RC_ALLOC_TEMP);
	if (!poly)
	{
		ACE_DEBUG((LM_ERROR, "rcBuildPolyMeshDetail: Out of memory 'poly' (%d).\n", nvp*3));
		return false;
	}
	...
	if (!hp.data)
	{
		ACE_DEBUG((LM_ERROR,"rcBuildPolyMeshDetail: Out of memory 'hp.data' (%d).\n", maxhw*maxhh));
		return false;
	}
	...
	if (!dmesh.meshes)
	{
		ACE_DEBUG((LM_ERROR,"rcBuildPolyMeshDetail: Out of memory 'dmesh.meshes' (%d).\n", dmesh.nmeshes*4));
		return false;
	}
	...
	if (!dmesh.verts)
	{
		ACE_DEBUG((LM_ERROR,"rcBuildPolyMeshDetail: Out of memory 'dmesh.verts' (%d).\n", vcap*3));
		return false;
	}
	dmesh.ntris = 0;
	...
	if (!dmesh.tris)
	{
		ACE_DEBUG((LM_ERROR,"rcBuildPolyMeshDetail: Out of memory 'dmesh.tris' (%d).", tcap*4));
		return false;
	}
	...
	for (int i = 0; i < mesh.npolys; ++i)
	{
		...
		if (dmesh.nverts+nverts > vcap)
		{
			...
			if (!newv)
			{
				ACE_DEBUG((LM_ERROR, "rcBuildPolyMeshDetail: Out of memory 'newv' (%d).\n", vcap*3));
				return false;
			}
			if (dmesh.nverts)
		...
		if (dmesh.ntris+ntris > tcap)
		{
			...
			if (!newt)
			{
				ACE_DEBUG((LM_ERROR,"rcBuildPolyMeshDetail: Out of memory 'newt' (%d).\n", tcap*4));
				return false;
			}
			if (dmesh.ntris)
...
bool rcMergePolyMeshDetails(rcPolyMeshDetail** meshes, const int nmeshes, rcPolyMeshDetail& mesh)
{
	...
	if (!mesh.meshes)
	{
		ACE_DEBUG((LM_ERROR,"rcBuildPolyMeshDetail: Out of memory 'pmdtl.meshes' (%d).\n", maxMeshes*4));
		return false;
	}
	...
	if (!mesh.tris)
	{
		ACE_DEBUG((LM_ERROR,"rcBuildPolyMeshDetail: Out of memory 'dmesh.tris' (%d).\n", maxTris*4));
		return false;
	}
