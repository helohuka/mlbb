...
static bool mergeAndFilterRegions(int minRegionArea, int mergeRegionSize,
								  ...
								  unsigned short* srcReg, rcIntArray& overlaps)
{
	...
	if (!regions)
	{
		ACE_DEBUG((LM_ERROR, "mergeAndFilterRegions: Out of memory 'regions' (%d).\n", nreg));
		return false;
	}
...
static bool mergeAndFilterLayerRegions(int minRegionArea,
									   unsigned short& maxRegionId,
									   rcCompactHeightfield& chf,
									   unsigned short* srcReg, rcIntArray& overlaps)
{
	...
	if (!regions)
	{
		ACE_DEBUG((LM_ERROR,  "mergeAndFilterLayerRegions: Out of memory 'regions' (%d).\n", nreg));
		return false;
	}
...
bool rcBuildDistanceField(rcCompactHeightfield& chf)
{
	...
	if (!src)
	{
		ACE_DEBUG((LM_ERROR, "rcBuildDistanceField: Out of memory 'src' (%d).\n", chf.spanCount));
		return false;
	}
	unsigned short* dst = (unsigned short*)rcAlloc(sizeof(unsigned short)*chf.spanCount, RC_ALLOC_TEMP);
	if (!dst)
	{
		ACE_DEBUG((LM_ERROR, "rcBuildDistanceField: Out of memory 'dst' (%d).\n", chf.spanCount));
		rcFree(src);
		return false;
	}
							...
							const int borderSize, const int minRegionArea, const int mergeRegionArea)
{
	...
	if (!srcReg)
	{
		ACE_DEBUG((LM_ERROR,  "rcBuildRegionsMonotone: Out of memory 'src' (%d).\n", chf.spanCount));
		return false;
	}
	...
	if (!sweeps)
	{
		ACE_DEBUG((LM_ERROR,  "rcBuildRegionsMonotone: Out of memory 'sweeps' (%d)\n.", nsweeps));
		return false;
	}
					...
					const int borderSize, const int minRegionArea, const int mergeRegionArea)
{
	...
	if (!buf)
	{
		ACE_DEBUG((LM_ERROR, "rcBuildRegions: Out of memory 'tmp' (%d).\n", chf.spanCount*4));
		return false;
	}
...
//		ctx->stopTimer(RC_TIMER_DIVIDE_TO_LEVELS);
	...
	if (overlaps.size() > 0)
	{
		ACE_DEBUG((LM_ERROR, "rcBuildRegions: %d overlapping regions.\n", overlaps.size()));
	}
...
bool rcBuildLayerRegions(rcCompactHeightfield& chf,
						 const int borderSize, const int minRegionArea)
{
	...
	rcIntArray overlaps;
	chf.maxRegions = id;
	if (!mergeAndFilterLayerRegions(minRegionArea, chf.maxRegions, chf, srcReg, overlaps))
		return false;
